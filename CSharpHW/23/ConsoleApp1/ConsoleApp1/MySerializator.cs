...
namespace ConsoleApp1
{
    class MySerializator
    {
        ...
        public static void XMLSerialization(MobileOperator user)
        {
            var formatter = new XmlSerializer(typeof(MobileOperator));
            using (FileStream fs = new FileStream("XMLMobileOperator.xml", FileMode.OpenOrCreate))
            {
                var watch = System.Diagnostics.Stopwatch.StartNew();
                formatter.Serialize(fs, user);
                watch.Stop();
